...
**ARSM** — это мощная CLI-утилита на базе Bash, предназначенная для глубокого низкоуровневого мониторинга Android-устройств. Проект эволюционировал из простого скрипта в комплексный инструмент диагностики, который понимает специфику различных SoC и архитектур Android 11-16. Скрипт ориентирован на энтузиастов, разработчиков ядер и опытных пользователей root-прав.

---

## 🛠 Ключевые возможности

* **Адаптивность к железу:** Автоматическое определение процессоров Qualcomm (Snapdragon Series) и Google Tensor (Pixel 6-10).
* **Интеллектуальный цикличный режим:** Режим мониторинга с компенсацией времени выполнения — интервалы всегда точные.
* **Глубокая проверка безопасности:** Проверка статусов Zygisk, Play Integrity, AVB и реального состояния загрузчика (обнаружение статуса "Fake Locked").
* **Анализ питания батареи:** Расчет мощности в Ваттах, определение протоколов быстрой зарядки (PD/QC) и мониторинг реверсивной зарядки.
...
* **Среда:** Termux, MT Manager или любой другой эмулятор терминала.
...
* `-b, --brief` — Краткий режим: вывод только при наличии статусов WARNING или CRITICAL.
...
* `-n, --no-logcat` — Пропустить тяжелый анализ системного лога для ускорения работы скрипта.

## 📊 Структура вывода
* **STORAGE:** Проверка раздела /data, износ памяти (UFS Life Time) и точки монтирования.
* **MEMORY:** Состояние RAM и использование ZRAM.
* **CPU/GPU:** Частоты по ядрам, статус троттлинга, загрузка видеоядра и часота обновления экрана.
* **BATTERY:** Здоровье АКБ, циклы, температура катушки и мгновенный ток.
* **NETWORK:** Тип соединения (Wi-Fi/Mobile Data), IP, оператор, задержка сети (Latency и Jitter).
...
* **Refresh Rate:** Данные берутся из SurfaceFlinger, поэтому на устройствах с динамическим частотой обновления экрана значение может быть усредненным.
* **Storage Health:** Доступно только на чипах, поддерживающих стандарт памяти UFS 2.1+ или eMMC с открытым life_time узлом.
